...
async function listarAdmin(req, res) {
  try {
    const garcons = await req.prisma.garcom.findMany({ orderBy: { nome: 'asc' }, select: SELECT_SEM_SENHA });
    res.json(garcons);
  } catch (err) {
...
async function criar(req, res) {
  try {
    ...
    const garcom = await req.prisma.garcom.create({
...
async function atualizar(req, res) {
  try {
    ...
    const garcom = await req.prisma.garcom.update({ where: { id }, data: dados, select: SELECT_SEM_SENHA });
    res.json(garcom);
  } catch (err) {
    if (err.code === 'P2002') {
      return res.status(409).json({ erro: 'Já existe um garçom com esse email' });
